...
@interface FlyHorizontalFlowLauyout()

@property (strong, nonatomic) NSMutableArray *allAttributes;
@property (strong, nonatomic) NSMutableDictionary *sectionPageDictionary;//每个section的页码的总数

@end

@implementation FlyHorizontalFlowLauyout
...
- (void)updateItemLayoutAttributes:(UICollectionViewLayoutAttributes *)attributes
{
    ...
    //1.初步计算水平方向上item的列数
    NSUInteger xItemCount = (collectionViewWidth - sectionInsets.left - sectionInsets.right + minimumInteritemSpacing)/(itemWidth + minimumInteritemSpacing);
...
//    }
    ...
    NSInteger yItemCount = (collectionViewHeight - sectionInsets.top - sectionInsets.bottom + minimumLineSpacing) / (itemHeight + minimumLineSpacing);
...
//    }
    ...
    if(remain == 0)//每行的第一个 + sectionInsets.left
    {
        xCellOffset = remain * itemWidth + sectionInsets.left;
    }
    else//其他 + sectionInsets.left + remain*minimumInteritemSpacing + sectionInsets.left
    {
